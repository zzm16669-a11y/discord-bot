...
def add_ticket_rating(guild_id: int, ticket_number: int, staff_id: int, rater_id: int,
                       stars: int, reason: str | None) -> None:
    with _lock:
        _conn.execute(
            "INSERT INTO ticket_ratings (guild_id, ticket_number, staff_id, rater_id, stars, reason, timestamp) "
            "VALUES (?,?,?,?,?,?,?)",
            (str(guild_id), ticket_number, str(staff_id), str(rater_id), stars, reason,
             datetime.now(timezone.utc).isoformat()),
        )
        _conn.commit()
